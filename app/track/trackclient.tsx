...
export default function TrackOrderClient() {
  ...
  const fetchOrder = useCallback(
    async (number: string, emailValue?: string, silent = false) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      fetchOrder(order.orderNumber, email, true);
    }, 15000);

    return () => clearInterval(interval);
  }, [order?.orderNumber, order?.status, email, fetchOrder]);
      ...
      <div className="mx-auto max-w-2xl">
        <div className="mb-8 text-center">
          ...
          <p className="mt-3 text-sm text-zinc-500">
            Enter your order number to see status. Updates every 15 seconds.
          </p>
          {autoRefreshing && (
            <p className="mt-2 text-xs font-bold text-green-700">Refreshing...</p>
          )}
        </div>
